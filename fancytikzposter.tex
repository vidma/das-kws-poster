...
%  \getcurrentrow{box}
  ...
  {Summary}%
  {%
  \textbf{Background:} Virtual data integration aims at providing a coherent interface for querying heterogeneous data sources (e.g. web services and proprietary systems) with minimum upfront effort in integration. %
  %This work explores various aspects of its usability.
Data is usually accessed through a structured query language, such as SQL, forcing users to learn the language and to get acquainted with data organization.
% thus negatively impacting the system’s usability.%

%\note{, given a keyword query,}
\textbf{Solution:} We present a keyword search system, which proposes a ranked list of structured queries along with explanations of their meanings. It operates mainly on the metadata, such as the constraints on inputs accepted by services.
...
It was developed as an integral part of the CMS data discovery service {\color{red} focusing on simplicity and capabilities of the interface}, and is currently available freely as open source.
...
%  }
  ...
  \plainblock[4]{($(box.south east)-(yshift)+(-10, 3.5)$)}%
  {27}{Finally, users see this} %
  {
  \newline
 ...
 %\coordinate (currenty) at ($(currentrow)+(xshift)-(yshift)$);
  ...
  %%%%%%%%%% ------------------------------------------ %%%%%%%%%%
  \blocknode%
  {Details: \color{red}Scoring \& Ranker}%
  {
	\input{./formulas}
  }
  ...
  % Munkres - how to guarantee top-K with (approx?) contextualization
  % HMM - how to know...
  ...
  %%%%%%%%%% ------------------------------------------ %%%%%%%%%%
  \blocknode {Future work \& Interesting problems}%
  {
    \input{future_work}
  }
