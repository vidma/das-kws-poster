...
\begin{tikzpicture}
  ...
  % }
  \addlogo[south west]{(0,-1.5+0.5)}{6cm}{images/vu_logo.png}
  \addlogo[south west]{(7,-1.5+1)}{6cm}{images/mif.png}
	%
  \addlogo[south east]{(3,-1.5+1)}{6cm}{images/nsf_logo.png}	
  \addlogo[south east]{(-2.5,-1.5+1)}{6cm}{images/cern_logo.png}
  \addlogo[south east]{(-9,-1.5+1)}{6cm}{images/cms_logo.png}
...
%  }
  ...
  \plainblock[0]{($(box.south west)-(yshift)+(4.3,-3.5)$)}%
  {9.2cm}{} %
  {\newline\vspace{-1.8cm}\newline
  	\textbf{Get this and more:}\vspace{0.5cm}
  	\newline
    \includegraphics[width=7cm]{images/qrcode.png}
  	\vspace{-0.7cm}
  }  
  ...
  \calloutblock[0]{($(kws_overv)+(-4.9,2.3)$)}%
  		{($(box.south east)-(yshift)+(-14, -3.5)$)} %
  {32} %
  {
  %\textbf{\color{blue}Result presentation:}
  %\newline
  	\includegraphics[width=1.0\textwidth]{images/ui_larger.png}
  	\vspace{-10pt}
  }
 ...
 %\coordinate (currenty) at ($(currentrow)+(xshift)-(yshift)$);
  ...
  {%    
    %\vspace{-0.3cm}
    \newline
    \textbf{Tokenized query} (intermediary result):
    \vspace{0.3cm}    \newline%
  ...
  {%    
\vspace{-0.5cm}
    \textbf{Entry points} (intermediary result):\newline
    \input{./entry_points}
        \vspace{-0.3cm}
