...
%  }
  ...
  \plainblock[4]{($(box.south east)-(yshift)+(-14, 3.5)$)}%
  ...
  \plainblock[0]{($(box.south west)-(yshift)+(xshift)+(-17,0)$)}%
  {8cm}{} %
  {\newline\vspace{-0cm}
 ...
 %\coordinate (currenty) at ($(currentrow)+(xshift)-(yshift)$);
  ...
  \setplainblockfillcolor{grey_light}
  \calloutblock[0]{($(kws_overv)+(-2,3)$)}%
  		{($(kws_overv)+(20, -0)$)}{40} %
  {%    
    \vspace{0.3cm}
    \textbf{Autocompletion to ease typing (keyword) queries (prototype)}\newline
     \includegraphics[width=35cm]{images/autocompl.png}
    }
  %% the coordinate (currenty) is used in the default placing of the next blocknode
  %\getcurrentrow{note}
  \getcurrentrow{note}
